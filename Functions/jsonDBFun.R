# This program will update the json files and create a little database for the shiny app
# data scientists update the model regularly and these JSON files are post-transformation metrics
...
# Function to code the json files on the fly

# Cleans up all the json files
# Creates a list with the following data frames:
# 1.  $incidence - disease priors
# 2.  $prevalence - risk factor priors
# 3.  $symptoms - symptom ?priors? - not sure if these are priors but whatever
# 4.  $duration - duration of symptoms for each incident disease
# 5.  $marginals - RRs for RF-D links



# There is a subfolder in this project called "diagnostic-engine-models"
# This ALL the data that go into the PGM
# It has flows, questions, priors, marginals, EVERYTHING, including historical models
# as I update this, I will need to rerun this function
# Part of this function will clone the github repo to get the latest versions
...
tempDB <- dbConnect(SQLite(), "jsonDB.DB") 
...
# Functions ---------------------------------------------------------------
  ...
  updateJson <- function() {
    ...
    lapply(c("US", "UK"), function(region) {
      ...
      dir.create(paste0("Data/json/",region), showWarnings = F)
      dir.create("data/json/archive", showWarnings = F)
      ...
      dir.create(file.path("data/json/archive", Sys.Date()), showWarnings = F) # warnings: already exists
...
rm(df)
  ...
  # Remove the diagnostic-engine-models data, it is large and can stay on github
  unlink("diagnostic-engine-models", recursive = T)
}
